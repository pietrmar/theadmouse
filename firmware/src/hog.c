...
static struct bt_gatt_attr hog_attrs[] = {
...
static struct bt_gatt_service hog_svc = BT_GATT_SERVICE(hog_attrs);

int hog_init(void)
{
	int ret = bt_gatt_service_register(&hog_svc);
	if (ret < 0)
		return ret;
...
void hog_push_report(int8_t btn, int8_t x, int8_t y)
{
